...
from osinter.errors import *  # noqa: F401
from osinter.core import CORE
from osinter.scanner import Preset
from osinter.core.helpers.async_helpers import get_event_loop
from osinter.core.helpers.misc import mkdir, rand_string, get_python_constraints


log = logging.getLogger("osinter.test.fixtures")


osinter_test_dir = Path("/tmp/.osinter_test")
mkdir(osinter_test_dir)
...
def tempwordlist(content):
    filename = osinter_test_dir / f"{rand_string(8)}"
...
def clean_default_config(monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.setattr("osinter.core.core.DEFAULT_CONFIG", clean_config)
        yield
...
def scan():
    ...
    osinter_scan = Scanner("127.0.0.1", modules=["ipneighbor"])
    yield osinter_scan

    loop = get_event_loop()
    loop.run_until_complete(osinter_scan._cleanup())
...
def events(scan):
    ...
    osinter_events.all = [  # noqa: F841
        osinter_events.localhost,
        osinter_events.ipv4,
        osinter_events.netv4,
        osinter_events.ipv6,
        osinter_events.netv6,
        osinter_events.domain,
        osinter_events.subdomain,
        osinter_events.email,
        osinter_events.open_port,
        osinter_events.protocol,
        osinter_events.ipv4_open_port,
        osinter_events.ipv6_open_port,
        osinter_events.url_unverified,
        osinter_events.ipv4_url_unverified,
        osinter_events.ipv6_url_unverified,
        osinter_events.url,
        osinter_events.ipv4_url,
        osinter_events.ipv6_url,
        osinter_events.url_hint,
        osinter_events.vulnerability,
        osinter_events.finding,
        osinter_events.vhost,
        osinter_events.http_response,
        osinter_events.storage_bucket,
        osinter_events.emoji,
    ]

    for e in osinter_events.all:
        e.scope_distance = 0

    return osinter_events
