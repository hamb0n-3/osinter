...
def run_osinter_multiprocess(queue):
    from osinter import Scanner
...
def test_osinter_multiprocess(osinter_httpserver):
    osinter_httpserver.expect_request("/").respond_with_data("test@blacklanternsecurity.com")

    queue = multiprocessing.Queue()
    events_process = multiprocessing.Process(target=run_osinter_multiprocess, args=(queue,))
...
def test_osinter_fastapi(osinter_httpserver):
    osinter_httpserver.expect_request("/").respond_with_data("test@blacklanternsecurity.com")
...
def start_fastapi_server():
    ...
    with suppress(KeyError):
        del env["OSINTER_TESTING"]
    python_executable = str(sys.executable)
    process = Popen(
        [python_executable, "-m", "uvicorn", "osinter.test.fastapi_test:app", "--port", "8978"], cwd=cwd, env=env
    )
    return process
