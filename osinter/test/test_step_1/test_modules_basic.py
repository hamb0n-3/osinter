...
from osinter.modules.report.base import BaseReportModule
...
async def test_modules_basic_checks(events, httpx_mock):
    ...
    from osinter.core.flags import flag_descriptions

    for flag in all_flags:
        assert flag in flag_descriptions, f'Flag "{flag}" not listed in osinter/core/flags.py'
        description = flag_descriptions.get(flag, "")
        assert description, f'Flag "{flag}" has no description in osinter/core/flags.py'

    await scan._cleanup()


@pytest.mark.asyncio
async def test_modules_basic_perhostonly(osinter_scanner):
    from osinter.modules.base import BaseModule
...
@pytest.mark.asyncio
async def test_modules_basic_perdomainonly(osinter_scanner, monkeypatch):
    per_domain_scan = osinter_scanner(
        "evilcorp.com",
...
@pytest.mark.asyncio
async def test_modules_basic_stats(helpers, events, osinter_scanner, httpx_mock, monkeypatch):
    from osinter.modules.base import BaseModule
...
@pytest.mark.asyncio
async def test_module_loading(osinter_scanner):
    scan2 = osinter_scanner(
        modules=list(available_modules),
