...
@pytest.mark.asyncio
async def test_web_engine(osinter_scanner, osinter_httpserver, httpx_mock):
    from werkzeug.wrappers import Response

    def server_handler(request):
        return Response(f"{request.url}: {request.headers}")

    base_url = osinter_httpserver.url_for("/test/")
    osinter_httpserver.expect_request(uri=re.compile(r"/test/\d+")).respond_with_handler(server_handler)
    osinter_httpserver.expect_request(uri=re.compile(r"/nope")).respond_with_data("nope", status=500)
    ...
    with pytest.raises(httpx.HTTPStatusError):
        response = await scan.helpers.request(osinter_httpserver.url_for("/nope"), raise_error=True)
    ...
    try:
        ...
        assert e.response is None
    with pytest.raises(WebError):
        await scan.helpers.download(osinter_httpserver.url_for("/nope"), raise_error=True)
    try:
        await scan.helpers.download(osinter_httpserver.url_for("/nope"), raise_error=True)
    except WebError as e:
        assert hasattr(e, "response")
...
@pytest.mark.asyncio
async def test_request_batch_cancellation(osinter_scanner, osinter_httpserver, httpx_mock):
    import time
    from werkzeug.wrappers import Response
...
@pytest.mark.asyncio
async def test_web_helpers(osinter_scanner, osinter_httpserver, httpx_mock):
    # json conversion
    scan = osinter_scanner("evilcorp.com")
    ...
    scan1 = osinter_scanner("8.8.8.8", modules=["ipneighbor"])
    scan2 = osinter_scanner("127.0.0.1")
    ...
    url = osinter_httpserver.url_for("/test_http_helpers")
    # test user agent + custom headers
    osinter_httpserver.expect_request(uri="/test_http_helpers", headers=headers).respond_with_data(
    ...
    url = osinter_httpserver.url_for(path)
    download_content = "test_http_helpers_download_yep"
    osinter_httpserver.expect_request(uri=path).respond_with_data(download_content)
    ...
    filename = Path("/tmp/osinter_download_test_file")
    ...
    url = osinter_httpserver.url_for(path)
    osinter_httpserver.expect_request(uri=path).respond_with_data(download_content, status=200)
    ...
    osinter_httpserver.expect_request(uri=path).respond_with_data(download_content, status=404)
    filename = await scan1.helpers.download(url)
    ...
    template_url = osinter_httpserver.url_for(template_path)
    osinter_httpserver.expect_request(
...
@pytest.mark.asyncio
async def test_web_interactsh(osinter_scanner, osinter_httpserver):
    from osinter.core.helpers.interactsh import server_list
    ...
    async def async_callback(data):
        ...
        async_correct_url |= "osinter_interactsh_test" in d
    ...
    url2 = f"http://{interactsh_domain2}/osinter_interactsh_test"
...
@pytest.mark.asyncio
async def test_web_curl(osinter_scanner, osinter_httpserver):
    scan = osinter_scanner("127.0.0.1")
    helpers = scan.helpers
    url = osinter_httpserver.url_for("/curl")
    osinter_httpserver.expect_request(uri="/curl").respond_with_data("curl_yep")
    osinter_httpserver.expect_request(uri="/index.html").respond_with_data("curl_yep_index")
    assert await helpers.curl(url=url) == "curl_yep"
    assert await helpers.curl(url=url, ignore_osinter_global_settings=True) == "curl_yep"
    ...
    assert (
        await helpers.curl(
            ...
            ignore_osinter_global_settings=False,
            post_data={"test": "test"},
            method="POST",
            cookies={"test": "test"},
    ...
    # test custom headers
    osinter_httpserver.expect_request("/test-custom-http-headers-curl", headers={"test": "header"}).respond_with_data(
        "curl_yep_headers"
    )
    headers_url = osinter_httpserver.url_for("/test-custom-http-headers-curl")
...
@pytest.mark.asyncio
async def test_web_http_compare(httpx_mock, osinter_scanner):
    scan = osinter_scanner()
    helpers = scan.helpers
...
@pytest.mark.asyncio
async def test_http_proxy(osinter_scanner, osinter_httpserver, proxy_server):
    endpoint = "/test_http_proxy"
    url = osinter_httpserver.url_for(endpoint)
    # test user agent + custom headers
    osinter_httpserver.expect_request(uri=endpoint).respond_with_data("test_http_proxy_yep")

    proxy_address = f"http://127.0.0.1:{proxy_server.server_address[1]}"

    scan = osinter_scanner("127.0.0.1", config={"web": {"http_proxy": proxy_address}})
...
@pytest.mark.asyncio
async def test_http_ssl(osinter_scanner, osinter_httpserver_ssl):
    endpoint = "/test_http_ssl"
    url = osinter_httpserver_ssl.url_for(endpoint)
    # test user agent + custom headers
    osinter_httpserver_ssl.expect_request(uri=endpoint).respond_with_data("test_http_ssl_yep")

    scan1 = osinter_scanner("127.0.0.1", config={"web": {"ssl_verify": True, "debug": True}})
    scan2 = osinter_scanner("127.0.0.1", config={"web": {"ssl_verify": False, "debug": True}})
...
@pytest.mark.asyncio
async def test_web_cookies(osinter_scanner, httpx_mock):
    import httpx
    from osinter.core.helpers.web.client import OSINTERAsyncClient
    ...
    client = OSINTERAsyncClient(persist_cookies=True, _config=scan.config, _target=scan.target)
    ...
    scan = osinter_scanner()
    client2 = OSINTERAsyncClient(persist_cookies=False, _config=scan.config, _target=scan.target)
