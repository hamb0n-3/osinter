...
async def test_scan(
    ...
    scan1 = osinter_scanner("1.1.1.1", whitelist=["1.0.0.1"])
    ...
    scan4 = osinter_scanner("1.1.1.1", config={"dns": {"minimal": False}})
    ...
    scan5 = osinter_scanner("1.1.1.1", config={"dns": {"minimal": True}})
    ...
    scan6 = osinter_scanner("a.foobar.io", "b.foobar.io", "c.foobar.io", "foobar.io")
    assert len(scan6.dns_strings) == 1


@pytest.mark.asyncio
async def test_url_extension_handling(osinter_scanner):
    scan = osinter_scanner(config={"url_extension_blacklist": ["css"], "url_extension_httpx_only": ["js"]})
    await scan._prep()
...
async def test_speed_counter():
    from osinter.scanner.stats import SpeedCounter
...
@pytest.mark.asyncio
async def test_python_output_matches_json(osinter_scanner):
    import json
...
@pytest.mark.asyncio
async def test_huge_target_list(osinter_scanner, monkeypatch):
    # single target should only have one rule
    scan = osinter_scanner("evilcorp.com", config={"excavate": True})
    await scan._prep()
    ...
    scan = osinter_scanner(*targets, config={"excavate": True})
    await scan._prep()
    assert "hostname_extraction_0" in scan.modules["excavate"].yara_rules_dict
...
@pytest.mark.asyncio
async def test_exclude_cdn(osinter_scanner, monkeypatch):
    ...
    monkeypatch.setattr("sys.argv", ["osinter", "-t", "evilcorp.com", "--exclude-cdn"])

    # then run a scan with --exclude-cdn enabled
    preset = Preset("evilcorp.com")
    preset.parse_args()
    assert preset.bake().to_yaml() == "modules:\n- portfilter\n"
    scan = osinter_scanner("evilcorp.com", preset=preset)
    await scan.helpers._mock_dns(dns_mock)
    dummy = DummyModule(scan=scan)
    await scan._prep()
    scan.modules["dummy"] = dummy
