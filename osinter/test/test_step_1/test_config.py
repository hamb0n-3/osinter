...
@pytest.mark.asyncio
async def test_config(osinter_scanner):
    ...
    scan1 = osinter_scanner("127.0.0.1", modules=["ipneighbor"], config=config)
    await scan1.load_modules()
    assert scan1.config.web.user_agent == "OSINTER Test User-Agent"
