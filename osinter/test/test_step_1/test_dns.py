...
from osinter.core.helpers.dns.helpers import extract_targets, service_record, common_srvs
...
@pytest.mark.asyncio
async def test_dns_engine(osinter_scanner):
    ...
    from osinter.core.helpers.dns.mock import MockResolver
...
@pytest.mark.asyncio
async def test_dns_resolution(osinter_scanner):
    scan = osinter_scanner("1.1.1.1")

    from osinter.core.helpers.dns.engine import DNSEngine
    ...
    scan2 = osinter_scanner("evilcorp.com", config={"dns": {"minimal": False}})
...
@pytest.mark.asyncio
async def test_wildcards(osinter_scanner):
    scan = osinter_scanner("1.1.1.1")
    helpers = scan.helpers

    from osinter.core.helpers.dns.engine import DNSEngine, all_rdtypes
...
"""
    ...
    scan = osinter_scanner(
        "osinter.fdsa.www.test.evilcorp.com",
    ...
    assert dns_names_by_host["osinter.fdsa.www.test.evilcorp.com"].tags == {
        "target",
        "subdomain",
        "in-scope",
        "txt-record",
    }
    assert dns_names_by_host["osinter.fdsa.www.test.evilcorp.com"].resolved_hosts == set()
    ...
    assert raw_records_by_host["osinter.fdsa.www.test.evilcorp.com"].tags == {"subdomain", "in-scope", "txt-record"}
    assert raw_records_by_host["osinter.fdsa.www.test.evilcorp.com"].resolved_hosts == set()
    ...
    assert raw_records_by_host["osinter.fdsa.www.test.evilcorp.com"].tags == {
        "subdomain",
        "in-scope",
        "txt-record",
        "txt-wildcard",
...
@pytest.mark.asyncio
async def test_wildcard_deduplication(osinter_scanner):
    custom_lookup = """
def custom_lookup(query, rdtype):
...
@pytest.mark.asyncio
async def test_dns_raw_records(osinter_scanner):
    from osinter.modules.base import BaseModule
    ...
    # scan without omitted event type
    scan = osinter_scanner("one.one.one.one", "1.1.1.1", config={"dns": {"minimal": False}, "omit_event_types": []})
    ...
    scan = osinter_scanner("one.one.one.one", config={"dns": {"minimal": False}, "omit_event_types": ["RAW_DNS_RECORD"]})
    await scan.helpers.dns._mock_dns(mock_records)
    dummy_module = DummyModule(scan)
    scan.modules["dummy_module"] = dummy_module
    events = [e async for e in scan.async_start()]
...
@pytest.mark.asyncio
async def test_dns_graph_structure(osinter_scanner):
    scan = osinter_scanner("https://evilcorp.com", config={"dns": {"search_distance": 1, "minimal": False}})
...
@pytest.mark.asyncio
async def test_hostname_extraction(osinter_scanner):
    scan = osinter_scanner("evilcorp.com", config={"dns": {"minimal": False}})
    await scan.helpers.dns._mock_dns(
        {
            "evilcorp.com": {
                "A": ["127.0.0.1"],
...
@pytest.mark.asyncio
async def test_dns_helpers(osinter_scanner):
    ...
    scan = osinter_scanner(config={"dns": {"brute_nameservers": brute_nameservers}})
