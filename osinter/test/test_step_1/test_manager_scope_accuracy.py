...
They are meant to be a thorough baseline of how different modules and OSINTER systems interact
Basically, if there is a small change in how scope works, dns resolution, etc., these tests are designed to catch it.
They will show you how your change affects osinter's behavior across a wide range of scans and configurations.
...
@pytest.mark.asyncio
async def test_manager_scope_accuracy(osinter_scanner, osinter_httpserver, osinter_other_httpservers, osinter_httpserver_ssl):
    """
    This test ensures that OSINTER correctly handles different scope distance settings.
    ...
    from osinter.modules.output.base import BaseOutputModule

    server_77, server_88, server_99, server_111, server_222, server_33 = osinter_other_httpservers

    osinter_httpserver.expect_request(uri="/").respond_with_data(response_data="<a href='http://127.0.0.77:8888'/>")
    ...
    async def do_scan(*args, _config={}, _dns_mock={}, scan_callback=None, **kwargs):
        scan = osinter_scanner(*args, config=_config, **kwargs)
    ...
    # I don't know what changed exactly, but it doesn't matter, either way is equally valid and osinter is meant to be flexible this way.
    assert 1 == len([e for e in events if e.type == "IP_RANGE" and e.data == "127.0.0.0/31" and e.internal is False and e.scope_distance == 0])
    assert 0 == len([e for e in events if e.type == "IP_ADDRESS" and e.data == "127.0.0.0"])
    assert 1 == len([e for e in events if e.type == "IP_ADDRESS" and e.data == "127.0.0.1" and e.internal is False and e.scope_distance == 0])
    assert 0 == len([e for e in events if e.type == "OPEN_TCP_PORT" and e.data == "127.0.0.0:8888"])
    ...
    events, all_events, all_events_nodups, graph_output_events, graph_output_batch_events = await do_scan(
        ...
        _dns_mock={"www.osintertest.notreal": {"A": ["127.0.1.0"]}, "test.notreal": {"A": ["127.0.0.1"]}},
    )
    ...
    assert 1 == len([e for e in events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is False and e.scope_distance == 1 and str(e.module) == "sslcert" and "affiliate" in e.tags])
    ...
    assert 1 == len([e for e in all_events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is False and e.scope_distance == 1 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events if e.type == "OPEN_TCP_PORT" and e.data == "www.osintertest.notreal:9999" and e.internal is True and e.scope_distance == 1 and str(e.module) == "speculate"])
    assert 1 == len([e for e in all_events if e.type == "DNS_NAME_UNRESOLVED" and e.data == "osintertest.notreal" and e.internal is True and e.scope_distance == 2 and str(e.module) == "speculate"])
    ...
    assert 1 == len([e for e in all_events_nodups if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is False and e.scope_distance == 1 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events_nodups if e.type == "OPEN_TCP_PORT" and e.data == "www.osintertest.notreal:9999" and e.internal is True and e.scope_distance == 1 and str(e.module) == "speculate"])
    assert 1 == len([e for e in all_events_nodups if e.type == "DNS_NAME_UNRESOLVED" and e.data == "osintertest.notreal" and e.internal is True and e.scope_distance == 2 and str(e.module) == "speculate"])
    ...
    for _graph_output_events in (graph_output_events, graph_output_batch_events):
        ...
        assert 1 == len([e for e in _graph_output_events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is False and e.scope_distance == 1 and str(e.module) == "sslcert"])
        assert 0 == len([e for e in _graph_output_events if e.type == "OPEN_TCP_PORT" and e.data == "www.osintertest.notreal:9999"])
        assert 0 == len([e for e in _graph_output_events if e.type == "DNS_NAME_UNRESOLVED" and e.data == "osintertest.notreal"])
    ...
    events, all_events, all_events_nodups, graph_output_events, graph_output_batch_events = await do_scan(
        ...
        _dns_mock={"www.osintertest.notreal": {"A": ["127.0.0.1"]}, "test.notreal": {"A": ["127.0.1.0"]}},
    )
    ...
    assert 1 == len([e for e in events if e.type == "DNS_NAME" and e.data == "test.notreal" and e.internal is False and e.scope_distance == 0 and str(e.module) == "sslcert"])
    assert 0 == len([e for e in events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal"])
    assert 0 == len([e for e in events if e.type == "OPEN_TCP_PORT" and e.data == "test.notreal:9999"])
    ...
    assert 1 == len([e for e in all_events if e.type == "DNS_NAME" and e.data == "test.notreal" and e.internal is False and e.scope_distance == 0 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is True and e.scope_distance == 3 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events if e.type == "OPEN_TCP_PORT" and e.data == "test.notreal:9999" and e.internal is True and e.scope_distance == 0 and str(e.module) == "speculate"])
    ...
    assert 1 == len([e for e in all_events_nodups if e.type == "DNS_NAME" and e.data == "test.notreal" and e.internal is False and e.scope_distance == 0 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events_nodups if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal" and e.internal is True and e.scope_distance == 3 and str(e.module) == "sslcert"])
    assert 1 == len([e for e in all_events_nodups if e.type == "OPEN_TCP_PORT" and e.data == "test.notreal:9999" and e.internal is True and e.scope_distance == 0 and str(e.module) == "speculate"])
    ...
    for _graph_output_events in (graph_output_events, graph_output_batch_events):
        ...
        assert 0 == len([e for e in graph_output_events if e.type == "DNS_NAME" and e.data == "www.osintertest.notreal"])
        assert 0 == len([e for e in graph_output_events if e.type == "OPEN_TCP_PORT" and e.data == "test.notreal:9999"])


@pytest.mark.asyncio
async def test_manager_blacklist(osinter_scanner, osinter_httpserver, caplog):

    osinter_httpserver.expect_request(uri="/").respond_with_data(response_data="<a href='http://www-prod.test.notreal:8888'/><a href='http://www-dev.test.notreal:8888'/>")
...
@pytest.mark.asyncio
async def test_manager_scope_tagging(osinter_scanner):
    scan = osinter_scanner("test.notreal")
