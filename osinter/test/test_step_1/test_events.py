...
from osinter.core.helpers.regexes import event_uuid_regex
...
async def test_events(events, helpers):
    ...
    from osinter.core.event import event_from_json
...
async def test_event_discovery_context():
    ...
    #  https://github.com/blacklanternsecurity/osinter/issues/1498
...
@pytest.mark.asyncio
async def test_event_web_spider_distance(osinter_scanner):
    ...
    scan = osinter_scanner(config={"web": {"spider_distance": 1}})
...
def test_event_magic():
    ...
    zip_file = Path("/tmp/.osintertestzipasdkfjalsdf.zip")
    ...
    assert event.data == {
        "path": "/tmp/.osintertestzipasdkfjalsdf.zip",
        "magic_extension": ".zip",
