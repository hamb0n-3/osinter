from ..osinter_fixtures import *  # noqa: F401


@pytest.mark.asyncio
async def test_target_basic(osinter_scanner):
    from radixtarget import RadixTarget
    from ipaddress import ip_address, ip_network
    from osinter.scanner.target import OSINTERTarget, ScanSeeds

    scan1 = osinter_scanner("api.publicapis.org", "8.8.8.8/30", "2001:4860:4860::8888/126")
    scan2 = osinter_scanner("8.8.8.8/29", "publicapis.org", "2001:4860:4860::8888/125")
    scan3 = osinter_scanner("8.8.8.8/29", "publicapis.org", "2001:4860:4860::8888/125")
    scan4 = osinter_scanner("8.8.8.8/29")
    scan5 = osinter_scanner()

    # test different types of inputs
    target = OSINTERTarget("evilcorp.com", "1.2.3.4/8")
    ...
    osintertarget = OSINTERTarget("http://1.2.3.4:8443", "bob@evilcorp.com")
    assert osintertarget.seeds.hosts == {ip_network("1.2.3.4"), "evilcorp.com"}
    assert osintertarget.whitelist.hosts == {ip_network("1.2.3.4"), "evilcorp.com"}
    assert {e.data for e in osintertarget.seeds.event_seeds} == {"http://1.2.3.4:8443/", "bob@evilcorp.com"}
    assert {e.data for e in osintertarget.whitelist.event_seeds} == {"1.2.3.4/32", "evilcorp.com"}

    osintertarget1 = OSINTERTarget("evilcorp.com", "evilcorp.net", whitelist=["1.2.3.4/24"], blacklist=["1.2.3.4"])
    osintertarget2 = OSINTERTarget("evilcorp.com", "evilcorp.net", whitelist=["1.2.3.0/24"], blacklist=["1.2.3.4"])
    osintertarget3 = OSINTERTarget("evilcorp.com", whitelist=["1.2.3.4/24"], blacklist=["1.2.3.4"])
    osintertarget5 = OSINTERTarget("evilcorp.com", "evilcorp.net", whitelist=["1.2.3.0/24"], blacklist=["1.2.3.4"])
    osintertarget6 = OSINTERTarget(
        "evilcorp.com", "evilcorp.net", whitelist=["1.2.3.0/24"], blacklist=["1.2.3.4"], strict_scope=True
    )
    osintertarget8 = OSINTERTarget("1.2.3.0/24", whitelist=["evilcorp.com", "evilcorp.net"], blacklist=["1.2.3.4"])
    osintertarget9 = OSINTERTarget("evilcorp.com", "evilcorp.net", whitelist=["1.2.3.0/24"], blacklist=["1.2.3.4"])

    # make sure it's a sha1 hash
    assert isinstance(osintertarget1.hash, bytes)
    assert len(osintertarget1.hash) == 20

    assert osintertarget1 == osintertarget2
    assert osintertarget2 == osintertarget1
    # 1 and 3 have different seeds
    assert osintertarget1 != osintertarget3
    assert osintertarget3 != osintertarget1
    # until we make them the same
    osintertarget3.seeds.add("evilcorp.net")
    assert osintertarget1 == osintertarget3
    assert osintertarget3 == osintertarget1

    # adding different events (but with same host) to whitelist should not change hash (since only hosts matter)
    osintertarget1.whitelist.add("http://evilcorp.co.nz")
    osintertarget2.whitelist.add("evilcorp.co.nz")
    assert osintertarget1 == osintertarget2
    assert osintertarget2 == osintertarget1

    # but seeds should change hash
    osintertarget1.seeds.add("http://evilcorp.co.nz")
    osintertarget2.seeds.add("evilcorp.co.nz")
    assert osintertarget1 != osintertarget2
    assert osintertarget2 != osintertarget1

    # make sure strict_scope is considered in hash
    assert osintertarget5 != osintertarget6
    assert osintertarget6 != osintertarget5

    # make sure swapped target <--> whitelist result in different hash
    assert osintertarget8 != osintertarget9
    assert osintertarget9 != osintertarget8

    # make sure duplicate events don't change hash
    target1 = OSINTERTarget("https://evilcorp.com")
    target2 = OSINTERTarget("https://evilcorp.com")
    assert target1 == target2
    target1.seeds.add("https://evilcorp.com:443")
    assert target1 == target2

    # make sure hosts are collapsed in whitelist and blacklist
    osintertarget = OSINTERTarget(
    ...
    with pytest.raises(TypeError):
        assert list(osintertarget) == ["http://evilcorp.com:8080/"]
    assert {e.data for e in osintertarget.seeds} == {"http://evilcorp.com:8080/"}
    assert {e.data for e in osintertarget.whitelist} == {"evilcorp.net:443", "http://evilcorp.net:8080/"}
    assert {e.data for e in osintertarget.blacklist} == {"http://evilcorp.org:8080/", "evilcorp.org:443"}

    # test org stub as target
    for org_target in ("ORG:evilcorp", "ORG_STUB:evilcorp"):
        scan = osinter_scanner(org_target)
    ...
    for user_target in ("USER:vancerefrigeration", "USERNAME:vancerefrigeration"):
        scan = osinter_scanner(user_target)
        events = [e async for e in scan.async_start()]
        assert len(events) == 3
        assert {e.type for e in events} == {"SCAN", "USERNAME"}

    # users + orgs + domains
    scan = osinter_scanner("USER:evilcorp", "ORG:evilcorp", "evilcorp.com")
    ...
    assert {e.data for e in osintertarget.seeds.event_seeds} == {
        "1.2.3.0/24",
        "http://www.evilcorp.net/",
        "bob@fdsa.evilcorp.net",
    }
    assert {e.data for e in osintertarget.whitelist.event_seeds} == {
        "evilcorp.com",
        "evilcorp.net",
        "bob@www.evilcorp.com",
    }
    assert {e.data for e in osintertarget.blacklist.event_seeds} == {
        "1.2.3.4",
        "4.3.2.0/24",
        "http://1.2.3.4/",
        "bob@asdf.evilcorp.net",
    }
    assert set(osintertarget.seeds.hosts) == {ip_network("1.2.3.0/24"), "www.evilcorp.net", "fdsa.evilcorp.net"}
    assert set(osintertarget.whitelist.hosts) == {"evilcorp.com", "evilcorp.net"}
    assert set(osintertarget.blacklist.hosts) == {ip_network("1.2.3.4/32"), ip_network("4.3.2.0/24"), "asdf.evilcorp.net"}
    assert osintertarget.hash == b"\xb3iU\xa8#\x8aq\x84/\xc5\xf2;\x11\x11\x0c&\xea\x07\xd4Q"
    assert osintertarget.scope_hash == b"f\xe1\x01c^3\xf5\xd24B\x87P\xa0Glq0p3J"
    assert osintertarget.seeds.hash == b"V\n\xf5\x1d\x1f=i\xbc\\\x15o\xc2p\xb2\x84\x97\xfeR\xde\xc1"
    assert osintertarget.whitelist.hash == b"\x8e\xd0\xa76\x8em4c\x0e\x1c\xfdA\x9d*sv}\xeb\xc4\xc4"
    assert osintertarget.blacklist.hash == b'\xf7\xaf\xa1\xda4"C:\x13\xf42\xc3,\xc3\xa9\x9f\x15\x15n\\'

    scan = osinter_scanner(
        "http://www.evilcorp.net",
        "1.2.3.0/24",
        "bob@fdsa.evilcorp.net",
...
@pytest.mark.asyncio
async def test_blacklist_regex(osinter_scanner, osinter_httpserver):
    from osinter.scanner.target import ScanBlacklist
    ...
    osinter_httpserver.expect_request(uri="/").respond_with_data(
        """
        <a href='http://127.0.0.1:8888/asdfevil333asdf'/>
        <a href='http://127.0.0.1:8888/logout.aspx'/>
    """
    )
    osinter_httpserver.expect_request(uri="/asdfevilasdf").respond_with_data("")
    osinter_httpserver.expect_request(uri="/logout.aspx").respond_with_data("")

    # make sure URL is detected normally
    scan = osinter_scanner("http://127.0.0.1:8888/", presets=["spider"], config={"excavate": True}, debug=True)
