...
@pytest.mark.asyncio
async def test_helpers_misc(helpers, scan, osinter_scanner, osinter_httpserver):
    ### URL ###
    bad_urls = (
    ...
    from osinter.core.helpers.regexes import url_regexes
    ...
    test_filesize = osinter_test_dir / "test_filesize"
    test_filesize.touch()
    assert test_filesize.is_file()
    assert helpers.filesize(test_filesize) == 0
    assert helpers.filesize(osinter_test_dir / "glkasjdlgksadlkfsdf") == 0
    ...
    scan1 = osinter_scanner(modules="ipneighbor")
...
@pytest.mark.asyncio
async def test_word_cloud(helpers, osinter_scanner):
    ...
    # saving and loading
    scan1 = osinter_scanner("127.0.0.1")
    ...
    # mutators
    from osinter.core.helpers.wordcloud import DNSMutator
...
async def test_ratelimiter(helpers):
    from osinter.core.helpers.ratelimiter import RateLimiter
...
async def test_async_helpers():
    import random
    from osinter.core.helpers.misc import as_completed
...
def test_liststring_valid_strings(helpers):
    assert helpers.chain_lists("hello,world,osinter") == ["hello", "world", "osinter"]
...
def test_liststring_invalidfnchars(helpers):
    with pytest.raises(ValueError) as e:
        helpers.chain_lists("hello,world,osinter|test", validate=True)
    assert str(e.value) == "Invalid character in string: osinter|test"
