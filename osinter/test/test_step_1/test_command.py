...
@pytest.mark.asyncio
async def test_command(osinter_scanner):
    scan1 = osinter_scanner()
    ...
    assert "/tmp/.osinter_test/tools" in path_parts
    run_lines = (await scan1.helpers.run(["env"])).stdout.splitlines()
    assert "OSINTER_WEB_USER_AGENT=OSINTER Test User-Agent" in run_lines
    for line in run_lines:
        if line.startswith("PATH="):
            path_parts = line.split("=", 1)[-1].split(":")
            assert "/tmp/.osinter_test/tools" in path_parts
    run_lines_sudo = (await scan1.helpers.run(["env"], sudo=True)).stdout.splitlines()
    assert "OSINTER_WEB_USER_AGENT=OSINTER Test User-Agent" in run_lines_sudo
    for line in run_lines_sudo:
        if line.startswith("PATH="):
            path_parts = line.split("=", 1)[-1].split(":")
            assert "/tmp/.osinter_test/tools" in path_parts
    run_live_lines = [l async for l in scan1.helpers.run_live(["env"])]
    assert "OSINTER_WEB_USER_AGENT=OSINTER Test User-Agent" in run_live_lines
    ...
    assert "OSINTER_WEB_USER_AGENT=OSINTER Test User-Agent" in run_live_lines_sudo
