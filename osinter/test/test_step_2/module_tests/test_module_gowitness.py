...
class TestGowitness(ModuleTestBase):
    ...
    home_dir = Path("/tmp/.osinter_gowitness_test")
    ...
    async def setup_after_prep(self, module_test):
        respond_args = {
            "response_data": """<html><head><title>OSINTER is life</title></head><body>
...
class TestGoWitnessLongFilename(TestGowitness):
    ...
    async def setup_after_prep(self, module_test):
        ...
        respond_args = {
            "response_data": "<html><head><title>OSINTER is life</title></head><body>OSINTER is life</body></html>",
            "headers": {"Server": "Apache/2.4.41 (Ubuntu)"},
        }
        module_test.set_expect_requests(request_args, respond_args)
