...
class TestMySQL(ModuleTestBase):
    ...
    async def setup_before_prep(self, module_test):
        process = await asyncio.create_subprocess_exec(
            ...
            "--rm",
            "-e",
            "MYSQL_ROOT_PASSWORD=osinterislife",
            "-e",
            "MYSQL_DATABASE=osinter",
            "-p",
            "3306:3306",
            "-d",
            "mysql",
        ...
        while True:
            try:
                conn = await aiomysql.connect(user="root", password="osinterislife", db="osinter", host="localhost")
                conn.close()
                break
    ...
    async def check(self, module_test, events):
        ...
        try:
            ...
            conn.close()
            process = await asyncio.create_subprocess_exec(
                "docker", "stop", "osinter-test-mysql", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
