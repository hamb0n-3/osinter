...
from osinter.core.helpers.misc import rand_string

log = logging.getLogger("osinter.test.modules")
...
class ModuleTestBase:
    ...
    class ModuleTest:
        def __init__(
            ...
            self.events = []
            self.log = logging.getLogger(f"osinter.test.{module_test_base.name}")
    ...
    async def module_test(
        self, httpx_mock, osinter_httpserver, osinter_httpserver_ssl, monkeypatch, request, caplog, capsys
    ):
        # Skip dastardly test if we're in the distro tests (because dastardly uses docker)
        if os.getenv("OSINTER_DISTRO_TESTS") and self.skip_distro_tests:
            pytest.skip("Skipping module_test for dastardly module due to OSINTER_DISTRO_TESTS environment variable")
    ...
    async def test_module_run(self, module_test):
        from osinter.core.helpers.misc import execute_sync_or_async
