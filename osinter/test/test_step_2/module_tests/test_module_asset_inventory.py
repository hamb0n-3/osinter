...
class TestAsset_Inventory(ModuleTestBase):
    targets = ["127.0.0.1", "osintertest.notreal"]
    ...
    async def setup_before_prep(self, module_test):
        ...
        await module_test.mock_dns(
            {
                "1.0.0.127.in-addr.arpa": {"PTR": ["www.osintertest.notreal"]},
                "www.osintertest.notreal": {"A": ["127.0.0.1"]},
            }
        )

    def check(self, module_test, events):
        assert any(e.data == "127.0.0.1:9999" for e in events), "No open port found"
        assert any(e.data == "www.osintertest.notreal" for e in events), "No DNS name found"
        filename = next(module_test.scan.home.glob("asset-inventory.csv"))
        with open(filename) as f:
            content = f.read()
            assert "www.osintertest.notreal,,,127.0.0.1" in content
        ...
        with open(filename) as f:
            content = f.read()
            assert "osintertest.notreal" in content
...
class TestAsset_InventoryEmitPrevious(TestAsset_Inventory):
    ...
    def check(self, module_test, events):
        assert any(e.data == "www.osintertest.notreal:9999" for e in events), "No open port found"
        assert any(e.data == "www.osintertest.notreal" for e in events), "No DNS name found"
        filename = next(module_test.scan.home.glob("asset-inventory.csv"))
        with open(filename) as f:
            content = f.read()
