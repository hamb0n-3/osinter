...
class TestPostgres(ModuleTestBase):
    ...
    async def setup_before_prep(self, module_test):
        process = await asyncio.create_subprocess_exec(
            ...
            "run",
            "--name",
            "osinter-test-postgres",
            "--rm",
            "-e",
            "POSTGRES_PASSWORD=osinterislife",
            "-e",
        ...
        while True:
            try:
                # Connect to the default 'postgres' database to create 'osinter'
                conn = await asyncpg.connect(
                    user="postgres", password="osinterislife", database="postgres", host="127.0.0.1"
                )
                await conn.execute("CREATE DATABASE osinter")
                await conn.close()
                break
    ...
    async def check(self, module_test, events):
        ...
        conn = await asyncpg.connect(user="postgres", password="osinterislife", database="osinter", host="127.0.0.1")
        ...
        try:
            ...
            await conn.close()
            process = await asyncio.create_subprocess_exec(
                "docker", "stop", "osinter-test-postgres", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
