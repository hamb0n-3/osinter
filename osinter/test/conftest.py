...
from osinter.core.helpers.interactsh import server_list as interactsh_servers
...
@pytest.fixture
def osinter_httpserver_allinterfaces():
...
class Interactsh_mock:
    def __init__(self, name):
        self.name = name
        self.log = logging.getLogger(f"osinter.interactsh.{self.name}")
...
def pytest_sessionfinish(session, exitstatus):
    ...
    loggers = [logging.getLogger("osinter")] + list(logging.Logger.manager.loggerDict.values())
    for logger in loggers:
        handlers = getattr(logger, "handlers", [])
        for handler in handlers:
            logger.removeHandler(handler)

    # Wipe out OSINTER home dir
    shutil.rmtree("/tmp/.osinter_test", ignore_errors=True)
