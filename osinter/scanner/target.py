...
from osinter.core.event import is_event
from osinter.core.event.helpers import EventSeed, BaseEventSeed
from osinter.core.helpers.misc import is_dns_name, is_ip, is_ip_type

log = logging.getLogger("osinter.core.target")


class BaseTarget(RadixTarget):
    """
    A collection of OSINTER events that represent a scan target.
    ...
    This class is inherited by all three components of the OSINTER target:
        - Whitelist
        - Blacklist
        - Seeds
    """
...
class ScanWhitelist(ACLTarget):
    """
    A collection of OSINTER events that represent a scan's whitelist.
    """
...
class ScanBlacklist(ACLTarget):
    """
    A collection of OSINTER events that represent a scan's blacklist.
    """
...
class OSINTERTarget:
    ...
    def blacklisted(self, host):
        ...
        Note that `host` can be a hostname, IP address, CIDR, email address, or any OSINTER `Event` with the `host` attribute.
