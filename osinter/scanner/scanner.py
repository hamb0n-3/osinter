...
from osinter import __version__
from osinter.core.event import make_event
from .manager import ScanIngress, ScanEgress
from osinter.core.helpers.misc import sha1, rand_string
from osinter.core.helpers.names_generator import random_name
from osinter.core.config.logger import GzipRotatingFileHandler
from osinter.core.multiprocess import SHARED_INTERPRETER_STATE
from osinter.core.helpers.async_helpers import async_to_sync_gen
from osinter.errors import OSINTERError, ScanError, ValidationError

log = logging.getLogger("osinter.scanner")


class Scanner:
    """A class representing a single OSINTER scan
    ...
    Attributes:
        ...
        config (omegaconf.dictconfig.DictConfig): OSINTER config (alias to `self.preset.config`).
        ...
        home (pathlib.Path): Base output directory of the scan (default: `~/.osinter/scans/<scan_name>`).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.preset.scan_name is None:
            ...
            while 1:
                ...
                if self.preset.output_dir is not None:
                    ...
                    home_path = self.preset.osinter_home / "scans" / scan_name
        ...
        if self.preset.output_dir is not None:
            ...
            self.home = self.preset.osinter_home / "scans" / self.name
        ...
        self.useragent = self.web_config.get("user_agent", "OSINTER")
    ...
    async def async_start(self):
        ...
        try:
            ...
            self.trace(f"Ran OSINTER {__version__} at {self.start_time}, command: {' '.join(sys.argv)}")
