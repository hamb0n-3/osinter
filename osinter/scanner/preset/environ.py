...
from osinter.core.helpers.misc import cpu_architecture, os_platform, os_platform_friendly
...
class OSINTEREnviron:
    def __init__(self, preset):
        self.preset = preset

    def flatten_config(self, config, base="osinter"):
        """
        Flatten a JSON-like config into a list of environment variables:
            {"modules": [{"httpx": {"timeout": 5}}]} --> "OSINTER_MODULES_HTTPX_TIMEOUT=5"
        """
    ...
    def prepare(self):
        ...
        # ensure osinter_tools
        environ["OSINTER_TOOLS"] = str(self.preset.core.tools_dir)
        add_to_path(str(self.preset.core.tools_dir), environ=environ)
        # ensure osinter_cache
        environ["OSINTER_CACHE"] = str(self.preset.core.cache_dir)
        # ensure osinter_temp
        environ["OSINTER_TEMP"] = str(self.preset.core.temp_dir)
        # ensure osinter_lib
        environ["OSINTER_LIB"] = str(self.preset.core.lib_dir)
        # export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:~/.osinter/lib/
        add_to_path(str(self.preset.core.lib_dir), k="LD_LIBRARY_PATH", environ=environ)

        # platform variables
        environ["OSINTER_OS_PLATFORM"] = os_platform()
        environ["OSINTER_OS"] = os_platform_friendly()
        environ["OSINTER_CPU_ARCH"] = cpu_architecture()

        # copy config to environment
        osinter_environ = self.flatten_config(self.preset.config)
        environ.update(osinter_environ)
