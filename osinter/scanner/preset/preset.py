...
from osinter.errors import *
from osinter.core import CORE
from osinter.core.helpers.misc import make_table, mkdir, get_closest_match


log = logging.getLogger("osinter.presets")
...
class Preset(metaclass=BasePreset):
    """
    A preset is the central config for a OSINTER scan. It contains everything a scan needs to run --
    ...
    Presets can be loaded from or saved to YAML. OSINTER has a number of ready-made presets for common tasks like
    ...
    Attributes:
        ...
        config (omegaconf.dictconfig.DictConfig): OSINTER config (alias to `core.config`)
        core (OSINTERCore): Local copy of OSINTERCore object.
        verbose (bool): Whether log level is currently set to verbose. When set, updates log level for all OSINTER log handlers.
        debug (bool): Whether log level is currently set to debug. When set, updates log level for all OSINTER log handlers.
        ...
        *targets,
        ...
        Args:
            *targets (str): Target(s) to scan. Types supported: hostnames, IPs, CIDRs, emails, open ports.
            ...
            output_dir (str or Path, optional): Directory to store scan output. Defaults to OSINTER home directory (`~/.osinter`).
            ...
            verbose (bool, optional): Set the OSINTER logger to verbose mode.
            debug (bool, optional): Set the OSINTER logger to debug mode.
            silent (bool, optional): Silence all stderr (effectively disables the OSINTER logger).
    ...
    @property
    def osinter_home(self):
        return Path(self.config.get("home", "~/.osinter")).expanduser().resolve()
    ...
    @property
    def preset_dir(self):
        return self.osinter_home / "presets"
    ...
    def bake(self, scan=None):
        """
        Return a "baked" copy of this preset, ready for use by a OSINTER scan.
        ...
        from osinter.scanner.target import OSINTERTarget

        baked_preset._target = OSINTERTarget(
    ...
    def helpers(self):
        if self._helpers is None:
            from osinter.core.helpers.helper import ConfigAwareHelper
    ...
    def environ(self):
        if self._environ is None:
            from .environ import OSINTEREnviron

            self._environ = OSINTEREnviron(self)
        return self._environ

    @property
    def args(self):
        if self._args is None:
            from .args import OSINTERArgs

            self._args = OSINTERArgs(self)
        return self._args
    ...
    def include_preset(self, filename):
        ...
        If the full path is not specified, OSINTER will look in all the usual places for it.
    ...
    def from_yaml_file(cls, filename, _exclude=None, _log=False):
        """
        Create a preset from a YAML file. If the full path is not specified, OSINTER will look in all the usual places for it.
