# This file contains SQLModel (Pydantic + SQLAlchemy) models for OSINTER events, scans, and targets.
...
log = logging.getLogger("osinter_server.models")
...
class OSINTERBaseModel(SQLModel):
...
class Event(OSINTERBaseModel, table=True):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class Scan(OSINTERBaseModel, table=True):
...
class Target(OSINTERBaseModel, table=True):
