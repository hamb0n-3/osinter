...
from osinter.logger import log_to_stderr
...
log = logging.getLogger("osinter.module_loader")

osinter_code_dir = Path(__file__).parent.parent


class ModuleLoader:
    """
    Main class responsible for preloading OSINTER modules.
    ...
    default_module_dir = osinter_code_dir / "modules"
    ...
    def preload(self, module_dirs=None):
        """Preloads all OSINTER modules.

        This function recursively iterates through each file in the module directories
        and preloads each OSINTER module to gather its meta-information and dependencies.

        Args:
            module_dir (str or Path): Directory containing OSINTER modules to be preloaded.
        ...
        Examples:
            >>> preload("/path/to/osinter_modules/")
            {
                "module1": {...},
                "module2": {...},
                ...
            ...
            for module_file in list_files(module_dir, filter=self.file_filter):
                ...
                if preloaded and module_cache_key == cache_key:
                    ...
                    if module_dir.name == "modules":
                        namespace = "osinter.modules"
                    else:
                        namespace = f"osinter.modules.{module_dir.name}"
                    try:
    ...
    def preload_module(self, module_file):
        """
        Preloads a OSINTER module to gather its meta-information and dependencies.

        This function reads a OSINTER module file, extracts its attributes such as
        events watched and produced, flags, meta-information, and dependencies.

        Args:
            module_file (str): Path to the OSINTER module file.

        Returns:
            dict: A dictionary containing meta-information and dependencies for the module.

        Examples:
            >>> preload_module("osinter/modules/wappalyzer.py")
            {
                "watched_events": [
                    "HTTP_RESPONSE"
                ],
    ...
    def load_module(self, module_name):
        """Loads a OSINTER module by its name.
    ...
    def ensure_config_files(self):
        ...
        if not files.config_filename.exists():
            log_to_stderr(f"Creating OSINTER config at {files.config_filename}")
        ...
        if not files.secrets_filename.exists():
            log_to_stderr(f"Creating OSINTER secrets at {files.secrets_filename}")
