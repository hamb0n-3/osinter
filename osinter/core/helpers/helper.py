...
from osinter.scanner.target import BaseTarget

log = logging.getLogger("osinter.core.helpers")
...
class ConfigAwareHelper:
    ...
    Attributes:
        config (dict): Configuration settings for the OSINTER scan instance.
        _scan (Scan): A OSINTER scan instance.
        osinter_home (Path): Home directory for OSINTER.
    ...
    def __init__(self, preset):
        self.preset = preset
        self.osinter_home = self.preset.osinter_home
        self.cache_dir = self.osinter_home / "cache"
        self.temp_dir = self.osinter_home / "temp"
        self.tools_dir = self.osinter_home / "tools"
        self.lib_dir = self.osinter_home / "lib"
        self.scans_dir = self.osinter_home / "scans"
    ...
    def temp_filename(self, extension=None):
        """
        temp_filename() --> Path("/home/user/.osinter/temp/pgxml13bov87oqrvjz7a")
        """
    ...
    @property
    def in_tests(self):
        return os.environ.get("OSINTER_TESTING", "") == "True"
