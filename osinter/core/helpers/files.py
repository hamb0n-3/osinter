...
log = logging.getLogger("osinter.core.helpers.files")
...
def tempfile(self, content, pipe=True):
    ...
    Examples:
        ...
        '/home/user/.osinter/temp/pgxml13bov87oqrvjz7a'

        >>> tempfile(["Another", "temp", "file"], pipe=False)
        '/home/user/.osinter/temp/someotherfile'
    """
...
def tempfile_tail(self, callback):
    ...
    try:
        os.mkfifo(filename)
        t = self.preset.core.create_thread(
            target=tail, args=(filename, callback), daemon=True, custom_name="osinter tempfile_tail()"
        )
        t.start()
    except Exception as e:
