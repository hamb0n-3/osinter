...
from ... import errors
from . import regexes as osinter_regexes
from .names_generator import random_name, names, adjectives  # noqa F401

log = logging.getLogger("osinter.core.helpers.misc")
...
def is_ptr(d):
    ...
    Examples:
        ...
        False
    """
    return bool(osinter_regexes.ptr_regex.search(str(d)))
...
def is_url(u):
    ...
    """
    u = str(u)
    for r in osinter_regexes.event_type_regexes["URL"]:
        if r.match(u):
            return True
    return False
...
def split_host_port(d):
    ...
    match = osinter_regexes.split_host_port_regex.match(d)
    if match is None:
    ...
    match = osinter_regexes.extract_open_port_regex.match(netloc)
    if match is None:
...
def is_dns_name(d):
    ...
    if is_ip(d):
        return False
    d = smart_decode(d)
    if osinter_regexes.dns_name_validation_regex.match(d):
        return True
    return False
...
def extract_words(data, acronyms=True, wordninja=True, model=None, max_length=100, word_regexes=None):
    ...
    if word_regexes is None:
        word_regexes = osinter_regexes.word_regexes
    words = set()
    data = smart_decode(data)
...
def latest_mtime(d):
    ...
    Examples:
        >>> latest_mtime("~/.osinter/scans/mushy_susan")
        1659016928.2848816
    """
    d = Path(d).resolve()
...
def clean_old(d, keep=10, filter=lambda x: True, key=latest_mtime, reverse=True, raise_error=False):
    ...
    Examples:
        >>> clean_old("~/.osinter/scans", filter=lambda x: x.is_dir() and scan_name_regex.match(x.name))
    """
    d = Path(d)
    if not d.is_dir():
        return
...
def extract_emails(s):
    ...
    """
    for email in osinter_regexes.email_regex.findall(smart_decode(s)):
        yield email.lower()
...
def extract_host(s):
    ...
    """
    s = smart_decode(s)
    match = osinter_regexes.extract_host_regex.search(s)
...
def make_table(rows, header, **kwargs):
    ...
    if not rows:
        rows = [[]]
    tablefmt = os.environ.get("OSINTER_TABLE_FORMAT", None)
