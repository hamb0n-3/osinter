...
from osinter.core.helpers.async_helpers import NamedLock
from osinter.core.helpers.dns.helpers import extract_targets
from osinter.core.helpers.misc import (
    is_ip,
    rand_string,
    parent_domain,
    domain_parents,
...
log = logging.getLogger("osinter.core.helpers.dns.engine.server")
...
class DNSEngine(EngineServer):
    ...
    @property
    def in_tests(self):
        return os.getenv("OSINTER_TESTING", "") == "True"
    ...
    async def _mock_dns(self, mock_data, custom_lookup_fn=None):
        ...
        def deserialize_function(func_source):
            assert self.in_tests, "Can only mock when OSINTER_TESTING=True"
