...
log = logging.getLogger("osinter.core.helpers.depsinstaller")


class DepsInstaller:
    CORE_DEPS = {
        # core OSINTER dependencies in the format of binary: package_name
    ...
    def __init__(self, parent_helper):
        ...
        # respect OSINTER's http timeout
    ...
    async def install(self, *modules):
        ...
        try:
            ...
            for m in modules:
                ...
                # take into consideration whether the venv or osinter home directory changes
                module_hash = self.parent_helper.sha1(
                    json.dumps(preloaded["deps"], sort_keys=True)
                    + self.venv
                    + str(self.parent_helper.osinter_home)
                    + os.uname()[1]
                ).hexdigest()
    ...
    async def pip_install(self, packages, constraints=None):
        ...
        # if no custom constraints are provided, use the constraints of the currently installed version of osinter
    ...
    def ensure_root(self, message=""):
        ...
        with self.ensure_root_lock:
            ...
            _sudo_password = os.environ.get("OSINTER_SUDO_PASS", None)
    ...
    async def install_core_deps(self):
        ...
        if to_install:
            playbook.append(
                {
                    "name": "Install Core OSINTER Dependencies",
        ...
        # run playbook
        if playbook:
            log.info(f"Installing core OSINTER dependencies: {','.join(sorted(to_install_friendly))}")
            self.ensure_root()
    ...
    def _setup_sudo_cache(self):
        if not self._sudo_cache_setup:
            ...
            os.environ["OSINTER_SUDO_KEYFILE"] = str(self._sudo_temp_keyfile.resolve())
