...
from osinter.core.engine import EngineClient
from osinter.core.helpers.misc import truncate_filename
from osinter.errors import WordlistError, CurlError, WebError
...
log = logging.getLogger("osinter.core.helpers.web")
...
class WebHelper(EngineClient):
    ...
    """
    Main utility class for managing HTTP operations in OSINTER. It serves as a wrapper around the OSINTERAsyncClient,
    ...
    Attributes:
        ...
        web_client (OSINTERAsyncClient): An instance of OSINTERAsyncClient for making HTTP requests.
    ...
    def AsyncClient(self, *args, **kwargs):
        ...
        try:
            ...
            from .client import OSINTERAsyncClient

            client = OSINTERAsyncClient.from_config(self.config, self.target, *args, persist_cookies=False, **kwargs)
    ...
    async def request(self, *args, **kwargs):
        ...
        used widely across OSINTER and within this helper class. Handles various exceptions and timeouts
    ...
    async def curl(self, *args, **kwargs):
        ...
        Args:
            *args: Variable length argument list for positional arguments. Unused in this function.
            ...
            ignore_osinter_global_settings (bool, optional): If True, ignores the global settings of OSINTER. Defaults to False.
        ...
        ignore_osinter_global_settings = kwargs.get("ignore_osinter_global_settings", False)

        if ignore_osinter_global_settings:
            http_timeout = 20  # setting 20 as a worse-case setting
            log.debug("ignore_osinter_global_settings enabled. Global settings will not be applied")
        else:
            http_timeout = self.parent_helper.web_config.get("http_timeout", 20)
            user_agent = self.parent_helper.web_config.get("user_agent", "OSINTER")
    ...
    def response_to_json(self, response):
        ...
        from osinter.core.helpers.misc import tagify, urlparse, split_host_port, smart_decode
