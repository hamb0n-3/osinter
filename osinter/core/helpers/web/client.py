...
log = logging.getLogger("osinter.core.helpers.web.client")
...
class OSINTERAsyncClient(httpx.AsyncClient):
    """
    A subclass of httpx.AsyncClient tailored with OSINTER-specific configurations and functionalities.
    ...
    Attributes:
        _osinter_scan (object): OSINTER scan object containing configuration details.
        _persist_cookies (bool): Flag to determine whether cookies should be persisted across requests.

    Examples:
        >>> async with OSINTERAsyncClient(_osinter_scan=osinter_scan_object) as client:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if headers is None:
            headers = {}
        # user agent
        user_agent = self._web_config.get("user_agent", "OSINTER")
