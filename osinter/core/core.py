...
from osinter.errors import OSINTERError
...
class OSINTERCore:
    """
    This is the first thing that loads when you import OSINTER.

    Unlike a Preset, OSINTERCore holds only the config, not scan-specific stuff like targets, flags, modules, etc.
    ...
    def __init__(self):
        ...
        self.logger
        self.log = logging.getLogger("osinter.core")

        self._prep_multiprocessing()

    def _prep_multiprocessing(self):
        import multiprocessing
        from .helpers.process import OSINTERProcess

        if SHARED_INTERPRETER_STATE.is_main_process:
            # if this is the main osinter process, set the logger and queue for the first time
            from functools import partialmethod

            OSINTERProcess.__init__ = partialmethod(
                OSINTERProcess.__init__, log_level=self.logger.log_level, log_queue=self.logger.queue
            )

        # this makes our process class the default for process pools, etc.
        mp_context = multiprocessing.get_context("spawn")
        mp_context.Process = OSINTERProcess
    ...
    def default_config(self):
        """
        The default OSINTER config (from `defaults.yml`). Read-only.
        """
        global DEFAULT_CONFIG
        if DEFAULT_CONFIG is None:
            self.default_config = self.files_config.get_default_config()
            # ensure osinter home dir
            if "home" not in self.default_config:
                self.default_config["home"] = "~/.osinter"
    ...
    def custom_config(self):
        """
        Custom OSINTER config (from `~/.config/osinter/osinter.yml`)
        """
    ...
    def files_config(self):
        """
        Get the configs from `osinter.yml` and `defaults.yml`
        """
        if self._files_config is None:
            from .config import files

            self.files = files
            self._files_config = files.OSINTERConfigFiles(self)
        return self._files_config

    def create_process(self, *args, **kwargs):
        if os.environ.get("OSINTER_TESTING", "") == "True":
            process = self.create_thread(*args, **kwargs)
        else:
            if SHARED_INTERPRETER_STATE.is_scan_process:
                from .helpers.process import OSINTERProcess

                process = OSINTERProcess(*args, **kwargs)
            else:
                import multiprocessing

                raise OSINTERError(f"Tried to start server from process {multiprocessing.current_process().name}")
        process.daemon = True
        return process

    def create_thread(self, *args, **kwargs):
        from .helpers.process import OSINTERThread

        return OSINTERThread(*args, **kwargs)

    @property
    def logger(self):
        self.config
        if self._logger is None:
            from .config.logger import OSINTERLogger

            self._logger = OSINTERLogger(self)
        return self._logger
