...
from osinter.core.helpers import (
    extract_words,
    is_domain,
    is_subdomain,
    is_ip,
...
log = logging.getLogger("osinter.core.event")


class BaseEvent:
    """
    Represents a piece of data discovered during a OSINTER scan.
    ...
    are the cornerstone of data exchange between OSINTER modules.
    ...
    def __init__(
        ...
        Attributes:
            ...
            scan (Scan, optional): OSINTER Scan object. Required unless _dummy is True. Defaults to None.
...
class FILESYSTEM(DictPathEvent):
    def __init__(self, *args, **kwargs):
        ...
        if self._data_path.is_file():
            ...
            from osinter.core.helpers.libmagic import get_magic_info, get_compression
...
def make_event(
    ...
    Parameters:
        ...
        scan (Scan, optional): OSINTER Scan object associated with the event.
        scans (List[Scan], optional): Multiple OSINTER Scan objects, primarily used for unserialization.
    ...
    Examples:
        ...
        >>> from osinter.core.event.base import make_event
