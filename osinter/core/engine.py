...
from osinter.errors import OSINTEREngineError
...
from osinter.core.helpers.misc import rand_string, in_exception_chain
...
class EngineBase:
    ...
    OSINTER makes use of this by spawning a dedicated engine for DNS and HTTP tasks.
    ...
    ERROR_CLASS = OSINTEREngineError

    def __init__(self, debug=False):
        self._shutdown_status = False
        self.log = logging.getLogger(f"osinter.core.{self.__class__.__name__.lower()}")
...
class EngineClient(EngineBase):
    """
    The client portion of OSINTER's RPC Engine.

    To create an engine, you must create a subclass of this class and also
    define methods for each of your desired functions.

    Note that this only supports async functions. If you need to offload a synchronous function to another CPU, use OSINTER's multiprocessing pool instead.
    ...
    def start_server(self):
        ...
        if SHARED_INTERPRETER_STATE.is_scan_process:
            ...
            self.process = CORE.create_process(
                ...
                kwargs=kwargs,
                custom_name=f"OSINTER {self.__class__.__name__}",
            )
            self.process.start()
            return self.process
        else:
            raise OSINTEREngineError(
    ...
    def server_process(server_class, socket_path, **kwargs):
        try:
            ...
            log = logging.getLogger("osinter.core.engine.server")
...
class EngineServer(EngineBase):
    """
    The server portion of OSINTER's RPC Engine.
    ...
    Examples:
        >>> from osinter.core.engine import EngineServer
        >>>
