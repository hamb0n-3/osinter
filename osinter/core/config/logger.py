...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # remove name
        if colored_record.name.startswith("osinter.modules."):
            colored_record.name = colored_record.name.split("osinter.modules.")[-1]
...
class OSINTERLogger:
    """
    The main OSINTER logger.

    The job of this class is to manage the different log handlers in OSINTER,
    ...
    def __init__(self, core):
        ...
        self.core_logger = logging.getLogger("osinter")
        self.core = core

        self.listener = None

        # if we haven't set up logging yet, do it now
        if "_OSINTER_LOGGING_SETUP" not in os.environ:
            os.environ["_OSINTER_LOGGING_SETUP"] = "1"
    ...
    def log_handlers(self):
        if self._log_handlers is None:
            ...
            if not mkdir(log_dir, raise_error=False):
                error_and_exit(f"Failure creating or error writing to OSINTER logs directory ({log_dir})")

            # Main log file
            main_handler = GzipRotatingFileHandler(f"{log_dir}/osinter.log", maxBytes=1024 * 1024 * 100, backupCount=100)

            # Separate log file for debugging
            debug_handler = GzipRotatingFileHandler(
                f"{log_dir}/osinter.debug.log", maxBytes=1024 * 1024 * 100, backupCount=100
            )
