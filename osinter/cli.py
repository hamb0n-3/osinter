...
from osinter.core.helpers.misc import chain_lists
...
www.blacklanternsecurity.com/osinter
"""
...
async def _main():
    ...
    log = logging.getLogger("osinter.cli")

    from osinter.scanner import Scanner
    from osinter.scanner.preset import Preset

    global scan_name

    try:
        # start by creating a default scan preset
        preset = Preset(_log=True, name="osinter_cli_main")
        # parse command line arguments and merge into preset
        try:
            preset.parse_args()
        except OSINTERArgumentError as e:
        ...
        if not options.dry_run:
            ...
            if sys.stdin.isatty():
                ...
                import re
                import fcntl
                from osinter.core.helpers.misc import smart_decode
